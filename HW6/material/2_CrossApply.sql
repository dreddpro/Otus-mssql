...
SELECT C.CustomerName, (SELECT TOP 1 OrderId
                ...
                ORDER BY O.OrderDate DESC, O.OrderID DESC) as LastOrder --последний ОДИН заказ  для этого CustomerID
FROM Sales.Customers C
ORDER BY C.CustomerName;

--последние 2 заказа, аналог Inner join = CROSS
SELECT C.CustomerName, O.OrderId as LastOrder_2 
FROM Sales.Customers C
CROSS APPLY (SELECT TOP 2 *
                FROM Sales.Orders O
                WHERE O.CustomerID = C.CustomerID
					AND OrderDate < '2014-01-01'
                ORDER BY O.OrderDate DESC, O.OrderID DESC) AS O --ДВА последних заказа  для этого CustomerID
ORDER BY C.CustomerName;

--последние 2 заказа, аналог Left join = OUTER
SELECT C.CustomerName, O.OrderId as LastOrder_2 
FROM Sales.Customers C
...
--Вычислим последнее значение и занесем его в исходную таблицу
UPDATE C
...
-- Упрощение кода за счет использования APPLY = 1
...
SELECT CA.PurchaseOrderMonth,
	COUNT(*) AS PurchaseCount
FROM Purchasing.PurchaseOrders AS P
CROSS APPLY (SELECT CAST(DATEADD(mm,DATEDIFF(mm,0,P.OrderDate),0) AS DATE)
				AS PurchaseOrderMonth) AS CA
...
-------------
-- Упрощение кода за счет использования APPLY = 2
create table #t
(
   ID int identity(1,1)
